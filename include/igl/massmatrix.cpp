  ...
  Eigen::SparseMatrix<Scalar>& M)
{
  ...
  Eigen::Matrix<int,Dynamic,1> MI;
  Eigen::Matrix<int,Dynamic,1> MJ;
  Eigen::Matrix<Scalar,Dynamic,1> MV;
  if(simplex_size == 3)
  {
    // Triangles
    // edge lengths numbered same as opposite vertices
    Eigen::Matrix<Scalar,Dynamic,3> l(m,3);
    // loop over faces
    for(int i = 0;i<m;i++)
    {
    ...
    Eigen::Matrix<Scalar,Dynamic,1> dblA;
    doublearea(l,dblA);
      ...
      case MASSMATRIX_TYPE_VORONOI:
        {
          ...
          Eigen::Matrix<Scalar,Dynamic,3> cosines(m,3);
          cosines.col(0) = 
          ...
          Eigen::Matrix<Scalar,Dynamic,3> barycentric = cosines.array() * l.array();
          normalize_row_sums(barycentric,barycentric);
          Eigen::Matrix<Scalar,Dynamic,3> partial = barycentric;
          partial.col(0).array() *= dblA.array() * 0.5;
          partial.col(1).array() *= dblA.array() * 0.5;
          partial.col(2).array() *= dblA.array() * 0.5;
          Eigen::Matrix<Scalar,Dynamic,3> quads(partial.rows(),partial.cols());
  ...
  }else if(simplex_size == 4)
  {
    ...
    for(int i = 0;i<m;i++)
    {
      ...
      Eigen::Matrix<Scalar,3,1> v0m3 = V.row(F(i,0)) - V.row(F(i,3));
      Eigen::Matrix<Scalar,3,1> v1m3 = V.row(F(i,1)) - V.row(F(i,3));
      Eigen::Matrix<Scalar,3,1> v2m3 = V.row(F(i,2)) - V.row(F(i,3));
