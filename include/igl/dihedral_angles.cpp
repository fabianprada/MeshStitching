  ...
  Eigen::PlainObjectBase<Derivedcos_theta>& cos_theta)
{
  ...
  assert(T.cols() == 4);
  Eigen::Matrix<typename Derivedtheta::Scalar,Dynamic,6> l;
  edge_lengths(V,T,l);
  Eigen::Matrix<typename Derivedtheta::Scalar,Dynamic,4> s;
  face_areas(l,s);
  ...
  Eigen::PlainObjectBase<Derivedcos_theta>& cos_theta)
{
  ...
  Eigen::Matrix<typename Derivedtheta::Scalar,Dynamic,6> H_sqr(m,6);
