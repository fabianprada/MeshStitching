...
template <
	...
	const int dim,
	const Eigen::PlainObjectBase<Derivedb> & b,
	const Eigen::VectorXi & softIndices,
	const Eigen::VectorXd & softWeights,
	ARAPData & data)
{
	using namespace std;
	using namespace Eigen;
	...
	//Set Vertex Weights Matrix
	SparseMatrix<double> VWeights;
	VWeights.resize(n, n);

	std::vector<bool> isFixed(n, false);
	for (int i = 0; i < b.size(); i++)isFixed[b[i]] = true;
	for (int i = 0; i < softIndices.size(); i++) if (isFixed[softIndices[i]]){
		printf("ERROR : Fixed index can not be soft!!\n");
		return false;
	}
	std::vector<Eigen::Triplet<double>> softTriplets;
	softTriplets.reserve(softIndices.size());
	for (int i = 0; i <softIndices.size(); i++){
		int vIndex = softIndices[i];
		softTriplets.push_back(Eigen::Triplet<double>(vIndex, vIndex, softWeights[i]));
	}
	VWeights.setFromTriplets(softTriplets.begin(), softTriplets.end());

	SparseMatrix<double> Q = (VWeights - L).eval();//Check .eval()
...
template <
	...
	const Eigen::VectorXi & softIndices,
	const Eigen::VectorXd & softWeights,
	const Eigen::MatrixXd & softConstraints,
	ARAPData & data,
	Eigen::PlainObjectBase<DerivedU> & U)
{
	using namespace igl;
	...
	while (iter < data.max_iter)
	{
		...
		for (int i = 0; i < softIndices.size(); i++){
			...
			for (int c = 0; c<data.dim; c++) Bcol[c*n + index] -= softConstraints(i, c)*softWeights[i];
		}

		assert(softConstraints.cols() == data.dim);
		assert(Bcol.size() == data.n*data.dim);
		for (int c = 0; c<data.dim; c++)
		{
			VectorXd Uc, Bc, bcc, Beq;
